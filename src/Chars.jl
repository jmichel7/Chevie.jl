...
function Base.show(io::IO, ::MIME"text/plain", ct::CharTable)
  println(io,ct)
  rows_label="centralizer"
  col_labels=string.(ct.centralizers)
  m=vcat(permutedims(ct.classnames),xrepr.(io,ct.irr))
  row_labels=vcat([""],ct.charnames)
  if haskey(ct,:orders)
    row_labels=vcat(["order"],row_labels)
    m=vcat(string.(ct.orders'),m)
    row_seps=[-1,1,2,2+size(ct.irr,1)]
  else
    row_seps=[-1,0,1,1+size(ct.irr,1)]
  end 
  showtable(io,m;row_labels,col_labels,dotzero=true,rows_label,row_seps)
end
