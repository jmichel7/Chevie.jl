...
"""
`eigen(powermap::Vector,char::Vector,class::Integer,order::Integer)

Let  `ρ`  be  a  representation  with  character  `char`, and let `w` be an
element  in the `class`-th  conjugacy class of  order `order`. The function
...
`powermap[p]` should be the index of the class of `wᵖ`.
"""
function LinearAlgebra.eigen(powermap::AbstractVector,char::AbstractVector,
  class::Integer,order::Integer)
  powers=char_powers_class(powermap,char,class,order)
