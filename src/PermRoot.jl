...
usually  elements of `K` are `Rational`s  or `Cyc`s. A *complex reflection*
is  an  element  `s∈  GL(V)`  of  finite  order whose fixed point set is an
hyperplane  (we  will  in  the  following  just  call  it a *reflection* to
...
Thus a reflection has a unique eigenvalue not equal to `1`, which is a root
of  unity. If  `K` is  a subfield  of the  real numbers, this eigenvalue is
necessarily equal to `-1`. A reflection is determined by a vector `r∈ V` (a
"root")  and a linear form  `rᵛ` on `V` (a  "corrot") such that `rᵛ(r)=1-ζ`
where `ζ` is the non-trivial eaigen value of the reflection. The reflection
is  then the map  `s` on `V`  defined by `s(x)=x-rᵛ(x)r`.  The root and the
coroot  for `s` are unique up to scaling the root and inversely scaling the
coroot.
...
*distinguished*;  it  is  a  canonical  generator  of  the  cyclic subgroup
centralizing  its  hyperplane.  The  generators  of  reflection  groups  we
construct  are always distinguished reflections. In a real reflection group
all reflections are distinguished.

We  deal in  this package  with finite  complex reflection groups. They are
groups where the following methods are defined:
...
`reflections(W)`  act as reflections. It is  much more efficient to compute
with permutation groups than with matrix groups, when possible. Information
sufficient  to determine  a particular  reflection representation is stored
for  such  groups  (see  `roots`,  `coroots`  and `reflrep`). Note that the
distinguished reflections are in bijection with the reflecting hyperplanes.

An  efficient  representation  that  we  use  for  computing with `W`, is a
