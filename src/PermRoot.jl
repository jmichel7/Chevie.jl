...
subfield of the real numbers, we get a real reflection which is necessarily
of order 2 and the non-trivial eigenvalue is equal to `-1`.
...
reflection with eigenvalue `E(d')` with `d'>d`. Such a reflection is called
*distinguished*;  we  take  it  as  the  canonical  generator of the cyclic
subgroup  it generates. The generators of reflection groups in this package
always   distinguished  reflections.   In  a   real  reflection  group  all
reflections are distinguished.

Reflection groups for us are groups <W> with the following fields (in
...
Shephard  and Todd  names which  range from  `G₄` to  `G₃₇`. They cover the
exceptional Coxeter groups, e.g., `coxgroup(:E,8)` is the same as `G₃₇`.

We provide functions to build  any finite reflection group, either
...
chosen  here  where  obtained  case-by-case; however, they satisfy
...
It  is a  theorem that `W` may be
realized as a reflection group over `k`. For almost all irreducible complex
reflection  groups, the generating matrices for  `W` we give have
coefficients  in `k`. Further, the set of  matrices for all elements of `W`
is  globally invariant  under the  Galois group  of `k/ℚ `, thus the Galois
action  induces automorphisms of  `W`. The exceptions  are `G_₂₂, G₂₇`
where  the matrices are in a degree two extension of `k` (this is needed to
have  a  globally  invariant  model,  see  [MarinMichel10])  and  some
...
matX(W::PRSG,i::Integer)=matX(W.parent,inclusion(W,i))
