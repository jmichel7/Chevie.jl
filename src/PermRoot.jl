...
`ζₛ`  not equal to `1`, which  is a root of unity  (if `K` is a subfield of
the  real numbers, `ζₛ` is necessarily equal  to `-1`). A reflection `s` is
determined  by a vector `r∈ V` (a *root*)  and a linear form `rᵛ` on `V` (a
*coroot*)  such that  `rᵛ(r)=1-ζ`. Then  `s` is  the map  on `V` defined by
`s(x)=x-rᵛ(x)r`.  The root and the coroot for  `s` are unique up to scaling
the root and inversely scaling the coroot.

A  complex reflection group `W` is a  group generated by a finite number of
complex reflections.

Since  when `W` contains  a reflection `s`  it contains its  powers, `W` is
always  generated by reflections `s` such  that `ζₛ==E(d)` where `d` is the
order  of `s`; we may in addition assume that `s` is not a power of another
reflection   with  larger  order  `d`.  Such   a  reflection  is  called  a
*distinguished*  reflection  of  `W`;  it  is  a canonical generator of the
cyclic  subgroup of  `W` centralizing  `Hₛ`. The  generators of the complex
reflection  groups we construct are  always distinguished reflections (in a
real reflection group all reflections are distinguished).

We  deal in  this module  with finite  complex reflection groups. For these
groups the following methods are defined:

`gens(W)`: the (distinguished) reflections which generate `W`

`reflections(W)`:  the  list  of  distinguished  reflections  of `W`, which
starts with gens(W).

Note  that `W` does *not* need to be  a matrix group. But by definition `W`
has  a  faithful  representation  (the  *reflection  representation*) where
`reflections(W)`  act as reflections (see `reflrep`).  Note that the set of
...
for  computing with `W`, is a permutation representation on a `W`-invariant
set  of root and coroot vectors for reflections  of `W`; that is, a set `R`
of  pairs `(r,rᵛ)∈ V×Vᵛ` invariant by  `W` and such that each distinguished
reflection in `W` is defined by some pair in `R` (see `reflection`). Trying
to  find a `W`-invariant  set implies that  there may be  several pairs for
each  reflection, differing by  roots of unity.  This generalizes the usual
construction  for Coxeter groups (the case `K=ℝ `) where to each reflection
of  `W`  is  associated  two  roots,  a  positive  and  a negative one. For
irreducible  complex reflection groups, there are at least as many roots on
a given line as the order of the center of `W`.
...
`PRG`),  or in terms of  the classification (see `ComplexReflectionGroup`).
The  output is a permutation group on set of roots . In the context e.g. of
Weyl  groups, one  wants to  describe the  particular root system chosen in
term  of the  traditional classification  of crystallographic root systems.
This  is  done  via  calls  to  the  function  `coxgroup`  (see  the module
`Weyl.jl`).  There is not yet  a general theory on  how to construct a nice
set  of roots for a non-real reflection  group; the roots chosen here where
obtained case-by-case; however, they satisfy several important properties:
...
well  as  `H₃`  and  `H₄`,  where  the  matrices  given  (the usual Coxeter
reflection representation over `K`) are not globally invariant (but one can
...
# print(title, " ");s=pad("\n", -length(title))
  ...
  elseif t.ST==11 println(io," "^2,c3," ",f(2)," ",t);println(io," /3\\");
    ...
    print(io,f(1)," "^3,f(3)," ",f(1,2,3),"=",f(2,3,1),"=",f(3,1,2))
   elseif t.ST==12 println(io,"  ",c2," ",f(2)," ",t);
    println(io," /4\\");println(io,c2," ",bar^2,c2)
    print(io,f(1)," "^3,f(3)," ",f(1,2,3,1),"=",f(2,3,1,2),"=",f(3,1,2,3))
   elseif t.ST==13 println(io,"  ",c2," ",f(1)," ",t)
    println(io," / \\");println(io,c2," ",bar^2,c2)
    print(io,f(3)," "^3,f(2)," ",f(2,3,1,2),"=",f(3,1,2,3)," ",f(1,2,3,1,2),"=")
    print(io,f(3,1,2,3,1))
  elseif t.ST==14 println(io,"  ₈");println(io,c2," ",bar,c3," ",t)
   print(io,f(1)," "^2,f(2))
  elseif t.ST==15 println(io,"  ",c2," ",f(1)," ",t);
   println(io," /5");println(io,c2," ",f(3));
   println(io," \\");print(io,"  ",c3," ",f(2)," ")
   print(io,f(1,2,3),"=",f(3,1,2)," ",f(2,3,1,2,1),"=",f(3,1,2,1,2))
  elseif t.ST==16 println(io,c5," ",bar^2,c5," ",t);print(io,f(1)," "^3,f(2))
  elseif t.ST==17 println(io,c2," ",tbar(2),c5," ",t);print(io,f(1)," "^3,f(2))
  elseif t.ST==18 println(io,c3," ",dbar^2,c5," ",t);print(io,f(1)," "^3,f(2))
  elseif t.ST==19 println(io," "^2,c3," ",f(2)," ",t)
                  println(io," /3\\");
                  println(io,c2," ",bar^2,c5)
    print(io,f(1)," "^3,f(3)," ",f(1,2,3),"=",f(2,3,1),"=",f(3,1,2))
  elseif t.ST==20 println(io,"  ₅");println(io,c3," ",bar,c3," ",t)
     print(io,f(1)," "^2,f(2))
  elseif t.ST==21 println(io,"  ₁₀");
    println(io,c2," ",bar^2,c3," ",t);print(io,f(1)," "^3,f(2))
  elseif t.ST==22 println(io,"  ",c2," ",f(2)," ",t);println(io," /5\\")
    println(io,c2," ",bar^2,c2)
   print(io,f(1)," "^3,f(3)," ",f(1,2,3,1,2),"=",f(2,3,1,2,3),"=",f(3,1,2,3,1))
  elseif t.ST==25 println(io,c3," ",bar^2,c3," ",bar^2,c3," ",t)
    print(io,f(1)," "^3,f(2)," "^3,f(3))
  elseif t.ST==26 println(io,c2," ",dbar^2,c3," ",bar^2,c3," ",t)
    print(io,f(1)," "^3,f(2)," "^3,f(3))
  elseif t.ST==32 println(io,c3," ",bar^2,c3," ",bar^2,c3," ",bar^2,c3," ",t)
    print(io,f(1)," "^3,f(2)," "^3,f(3)," "^3,f(4))
  else getchev(t,:PrintDiagram,t.indices,repr(t;context=io))
  end
...
list of same length as `W.roots` giving the corresponding reflections.
In particular this list is much longer than `unique(reflections(W))`.
"""
