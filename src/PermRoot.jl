...
is  an hyperplane, called the reflecting hyperplane  of `s` (we will in the
following just call it a *reflection* to abbreviate; in some literature the
term  reflection is only employed when the  order is 2 and the more general
case  is  called  a  *pseudo-reflection*).  Thus  a reflection has a unique
eigenvalue  `ζ` not equal  to `1`, which  is a root  of unity. If  `K` is a
subfield of the real numbers, this eigenvalue is necessarily equal to `-1`.
A  reflection is determined by a vector `r∈ V` (a "root") and a linear form
`rᵛ`  on `V` (a "coroot") such that `rᵛ(r)=1-ζ`. The reflection `s` is then
the  map on `V` defined by `s(x)=x-rᵛ(x)r`. The root and the coroot for `s`
...
called  *distinguished*; it is a canonical generator of the cyclic subgroup
of `W` centralizing its reflecting hyperplane. The generators of reflection
groups  we  construct  are  always  distinguished  reflections  (in  a real
reflection group all reflections are distinguished).

We  deal in  this package  with finite  complex reflection groups. They are
groups for which the following methods are defined:
...
Note  that `W` does *not* need to be a matrix group. But `W` has a faithful
representation    (called    the    *reflection    representation*)   where
`reflections(W)`  act as  reflections. Note  that the  set of distinguished
reflections are in bijection with the reflecting hyperplanes.

It is  much more efficient to compute
with permutation groups than with matrix groups, when possible.
The  efficient  representation  that  we  use  for  computing with `W`, is a
permutation  representation  on  a  `W`-invariant  set  of  root and coroot
vectors  for reflections of `W`; that is, a set `R` of pairs `(r,rᵛ)∈ V×Vᵛ`
invariant  by `W` and such each  distinguished reflection in `W` is defined
by some pair in `R` (see `reflection`). There may be several pairs for each
reflection,  differing  by  roots  of  unity.  This  generalizes  the usual
...
irreducible  complex  reflection  groups,  since  we  want  a  set  of root
invariant  by `W`, there are at least as  many roots on a given line as the
order of the center of `W`.
...
a  list of  roots and  coroots defining  the generating  reflections, or in
terms  of the classification. The  output is a permutation  group on set of
roots (see `ComplexReflectionGroup` and `PRG`). In the context e.g. of Weyl
...
done  via  calls  to  the  function  `coxgroup`  (see the chapter on finite
Coxeter  groups). There is not  yet a general theory  on how to construct a
...
thesis).  This has been known f/home/jmichel/or a long  time in the case `K=ℚ `, the case
...
function cartan(W::PermRootGroup)
  get!(W,:cartan)do
    improve_type(cartan(W,1:ngens(W)))
  end
end
