...
export PermRootGroup, PRG, PRSG, 
 ...
 position_class, reflength, bipartite_decomposition, torus_order, rank
...
rank(W::PermRootGroup)=length(W.roots)[1]
...
function root_representatives(W::PRG)
...
function independent_roots(W::PRG)::Vector{Int}
  gets(W,:indeproots) do W
    echelon(permutedims(hcat(W.roots...)))[2]
  end
end

function baseX(W::PRG{T})::Matrix{T} where T
  gets(W,:baseX) do W
...
function reflection_subgroup(W::PRG,I::AbstractVector{Int})
  ...
  PRSG(G,inclusion,restriction,W,Dict{Symbol,Any}())
end
...
function root_representatives(W::PRSG)
...
function independent_roots(W::PRSG)::Vector{Int}
  gets(W,:indeproots) do W
