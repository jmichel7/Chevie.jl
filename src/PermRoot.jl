...
We deal in this module with finite complex reflection groups. Note that `W`
does  *not* need to be a matrix group. But by definition `W` has a faithful
representation (the *reflection representation*) where `reflections(W)` act
as  reflections (see [`reflrep`](@ref)). Note that the set of distinguished
...
reflection in `W` is defined by some pair in `R` (see [`reflection`](@ref)).
Trying  to find a `W`-invariant set implies that there may be several pairs
