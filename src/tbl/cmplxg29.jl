...
chevieset(:G29, :classwords, map(x->
...
chevieset(:G29, :classinfo, Dict{Symbol, Any}(
  :classwords => chevieget(:G29, :classwords),
...
chevieset(:G29,:sparse_fakedegrees,[[1,0],[1,40],[1,4,1,8,1,12,1,16],
 ...
 (-1, [125, 202, 599]), (1, [8, 693, 876]), (y^-1, [255])])
  function f22(x,y)
    R=WGraph2Representation([[[1,3,4],[1,2,4],[1,2,3],[1,4],[1,3],[2],[1,2,3],
      ...
      [6,15,0,-y],[7,8,y,-x],[7,9,y//x,-x^2],[7,15,y,0],[8,12,y,-x],
      [8,13,-y,0],[9,11,-y*x,1],[9,12,y^2,0],[9,14,-y+x,0],[10,11,x,-y],
      [10,12,-y,x],[10,14,0,y*x],[11,13,-x,0],[12,15,0,y^2//x],
      [13,14,-1,y*x],[13,15,x,-y],[14,15,x^2,-y//x]]],[x,y])*y^0*x^0
    [R[1], R[2],improve_type(inv(R[4]//1)*R[3]*R[4]),R[4]]
  end
  function f27(x,y,v)
...
chevieset(:G29, :UnipotentCharacters,
  ...
  mkcuspidal("G_{29}",55,1),
  mkcuspidal("G_{29}",56,E(5)),
  mkcuspidal("G_{29}",57,E(5,2)),
  mkcuspidal("G_{29}",58,E(5,3)),
  mkcuspidal("G_{29}",59,E(5,4)),
  mkcuspidal("G_{29}",41,E(8);qeig=1//2),
...
chevieset(:G29, :BasicDerivations, ()->(x, y, z, t)->
 [4x 8*y^2 12z+1//40*x*y 20t+1//10*y*z;
...
chevieset(:G29, :Discriminant, ()->(x, y, z, t)->
          -442368*t*x*y*z^4+4*t*x*y^4*z^2-4736*t*x^2*y^2*z^3+4608*x*y^2*z^5+
