...
const decs=Dict(
  ...
  12=>[[1,2],[3,4],[1,3],[2,4],[1,4],[2,3],[1],[2],[3],[4]],
  15=>[[1,3,6,8],[2,4,5,7],[1,3,4,7],[2,5,6,8],[1,3],[4,7],[6,8],[2,5]],
  16=>[[1,4,5,8],[2,3,6,7]],
  20=>[[1,4,5,8],[2,3,6,7],[1,4,6,7],[2,3,5,8]],
  21=>[[1,3,6,8,9,11],[2,4,5,7,10,12]],
  24=>[[1,3,5,7],[2,4,6,8],[1,3,6,8],[2,4,5,7],[1,2,7,8],[3,4,5,6],[1,4,6,7],
       [2,3,5,8],[3,5],[1,7],[2,8],[4,6]],
  30=>[[1,3,6,8],[2,4,5,7],[1,2,4,6],[3,5,7,8],[3,8],[5,7],[1,6],[2,4]],
  42=>[[1,4,6,8,10,11],[2,3,5,7,9,12]]
)

# returns list of subsets of primitive_roots(d) wich have a `name` Φ^i
@memoize function dec(d::Int)
  if d==1 || d==2 return [[1]] end
  dd=[prime_residues(d)]
  if (r=primitiveroot(d))!==nothing
    for a in 0:1
      push!(dd,powermod.(r,(0:2:phi(d)-2).+a,d))
    end
  end
  if haskey(decs,d) append!(dd,map(x->dd[1][x],decs[d])) end
  dd
end
