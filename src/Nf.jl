...
@GapObj struct NumberField
  ...
  gens::Vector{<:Cyc}
  base::Vector{<:Cyc}
  stabilizer::Vector{Int} # galois-stabilize gens in prime_residues(conductor)
  conductor::Int
end
...
function NF(gens::AbstractVector{<:Cyc})
  ...
  stabilizer=filter(x->galois.(gens,x)==gens,prime_residues(cond))
  NF(cond,stabilizer,gens)
end

NF(gens::AbstractVector)=isempty(gens) ? CF(1) : NF(Cyc.(gens))

Base.:(==)(a::NumberField,b::NumberField)=conductor(a)==conductor(b) && a.stabilizer==b.stabilizer
...
function Base.in(c::Union{Cyc,Root1,Integer,Rational{<:Integer}},F::NumberField)
  (conductor(F)%conductor(c)==0) && all(i->galois(c,i)==c,F.stabilizer[2:end])
end
...
function CF(N)
  ...
  NumberField(totient(N),[Cyc(E(N))],
              ...
              [1],conductor(E(N)),Dict{Symbol,Any}())
end
...
struct NFAut
  ...
  function NFAut(F::NumberField,g::Union{Integer,Mod})
    ...
    new(F,minimum(map(x->x*g,F.stabilizer)))
  end
end
...
CyclotomicNumbers.galois(x::Cyc,y::Mod)=galois(x,Int(y.val))
...
function NF(N::Integer,stabilizer::AbstractVector{<:Integer},gens=nothing)
  ...
  NN=1
  for gen in gens_prime_residues(N)
    if gen.prime==2
      if gen.exponent<3
  ...
  res=NumberField(d,gens,base,stabilizer,N,Dict{Symbol,Any}())
