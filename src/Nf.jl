...
A  number field `K` is printed by  giving the conductor `c` of the smallest
cyclotomic field `F` containing it, and generators of the stabilizer of `K`
in  the  galois  group  of  `F`.  These  generators  are  elements  of  the
multiplicative group of `ℤ/cℤ`. Above `NF(5,[-1₅])` represents the subfield
of `CF(5)` stable by complex conjugacy.
...
NF(c::Int,gens::Vector{<:Mod})=NF(c,Group(gens))
NF(c::Int,gens::Vector{Int})=NF(c,Mod.(gens,c))
...
Number   field  which  is  the  fixed  field  of  the  subgroup  `stab`  of
`galois(CF(N))`  in `CF(N)`. `stab` should not inject in the multiplicative
group of a proper divisor of `N`.
"""
