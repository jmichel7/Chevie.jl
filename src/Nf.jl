"""
The  only *number fields* that we can handle at the moment are subfields of
cyclotomic fields, that is whose elements are `Cyc`s. For example, `ℚ (√5)`
is  a number field that  is not cyclotomic but  contained in the cyclotomic
field `ℚ (ζ₅)`.
...
julia> conductor(K) # no. of smallest cyclotomic field containing K
5
...
of  unity  to  the  power  -2.  Thus `NF(5,-1₅)` represents the subfield of
...
cyclotomic  `βᵢ=∑_{j∈ bᵢ} ζₙʲ` such that the  `βᵢ` form an integral base of
the number field `NF(n,stab)`.
...
The  *Galois automorphisms* of the cyclotomic  field `CF(n)` are the linear
extensions `σₖ` of raising `n`-th roots of unity to some power `k` prime to
`n`.  If  `F`  is  a  subfield  of  `CF(n)`,  a Galois automorphism `σₖ` is
replaced  by  a  canonical  representative  `σₗ`  of  its  orbit modulo the
stabilizer  of `F` in the Galois  group `galois(CF(n))`. This is the result
of `Aut(F,k)`. The number `k` can be given as an integer or as `Mod(k,n)`.
...
"""
`galois(F::NumberField)` galois of `F` over `ℚ`
...
"""
`NF(N, stab)`
fixed field of the subgroup `stab` of `galois(CF(N))` in `CF(N)`.
`stab` should not live in the multiplicative group of a proper divisor of `N`.
"""
