...
`2`-symbols  where introduced  by [Lusztig1977](biblio.htm#Lus77)  and more
general  `e`-symbols by  [Malle1995](biblio.htm#Mal95). An  `e`-symbol is a
vector  `S=[S₁,…,Sₑ]`  of  β-sets,  taken  modulo  the equivalence relation
generated by the simultaneous elementary shift of all β-sets, and by cyclic
permutations  of `S`; in  the particular case  where `e=2`, `S`  is thus an
unordered pair of β-sets. `S` is a *normalized symbol* if `0` is not in the
