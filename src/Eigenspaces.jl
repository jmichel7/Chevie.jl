...
that  `H^w` is a "standard" parabolic subgroup of `W` (errors if `H` is not
parabolic).

Standard  parabolic subgroups of `W` are those generated by a subset of the
generators,  except that for complex reflection groups, standard parabolics
include the result of `parabolic_reps(W)`.
"""
...
and  `N`  is  isomorphic  to  the  quotient ``R=N_W(W_J)/W_J``, which has a
natural   reflection  representation  on   ``X(ZL_J/ZG)``,  using  that  by
[lus76](@cite),  the images of the roots of  `W` in ``X(ZL_J)`` form a root
system.
...
some extra attributes reflecting its origin
  ...
  - `R.fromparent(w)` is a function mapping `w∈  N` to its image in `R` by
    the isomorphism inverse to `.toparent`.
```julia-repl
julia> W=coxgroup(:E,8)
E₈
