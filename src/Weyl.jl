...
integral    matrix.   In   general,    all   finite-dimensional   (complex)
representations  of a finite Coxeter group  can be realized over the (real)
subfield generated by the entries of the Cartan matrix.

An integral Cartan matrix can be encoded in a *Dynkin diagram*, a tree with
weighted  edges and an orientation on edges  of even weight >2, as follows.
The  vertices  are  indexed  by  the  simple  reflections; an edge is drawn
between  `s` and `t` if the order `mₛₜ`  of `st` is greater than `2` and is
given  the  weight  `mₛₜ`.  These  weights  are encoded by drawing the edge
single  for weight  3, double  for weight  4 and  triple for  weight 6. The
arrows  indicate the  relative root  lengths (going  from the longer to the
shorter  root) which  may differ  between different  orbits of  `W` on `R`.
Alternately  the Dynkin diagram  can be obtained  from the Cartan matrix as
follows: if `Cᵣₛ` and `Cₛᵣ` are integers such that `|Cₛᵣ|≥|Cᵣₛ|=1` there is
an  edge of weight `|Cₛᵣ|` from `r` to `s` with an arrow pointing to `s` if
`|Cₛᵣ|>1`.  Note  that  the  Cartan  matrices  we  consider  here  are  not
necessarily  symmetric,  contrary  to  the  Cartan  matrices  we considered
describing  the reflection representation of a general Coxeter group; being
symmetric corresponds to all roots being taken of the same length.
...
underlying  Coxeter system  group: we  see that  the root systems `B_n` and
`C_n`  correspond to the same Coxeter group (the Coxeter diagram is defined
by  the [`coxeter_matrix`](@ref)). Weyl groups can also be characterized as
the finite Coxeter groups such that all off-diagonal entries of the Coxeter
matrix are in `{2,3,4,6}`.

Here  are the Coxeter diagrams for the  finite Coxeter groups which are not
rational (`I₂(e)` is not rational if `e∉ {2,3,4,6}`).
```
       e        5         5
I₂(e) O—O   H₃ O—O—O  H₄ O—O—O—O
      1 2      1 2 3     1 2 3 4
```
The  function [`cartan`](@ref) gives  the cartan matrix  for an irreducible
root system
...
For  non-irreducible root systems, we can  get their Cartan matrix by using
that,   given   Cartan   matrices   `c₁,…,cᵣ`,   their  matrix  direct  sum
(corresponding  to the  orthogonal direct  sum of  the root systems) can be
obtained by `cat(c₁,…,cᵣ;dims=[1,2])`.
...
The function [`rootdatum`](@ref) takes as arguments a matrix `r` whose rows
represents the elements of `Π` (given by their coordinates in the canonical
basis of `V`) and another matrix `cr` whose rows are the coordinates of the
corresponding coroots and returns a `FiniteCoxeterGroup`. Such an object is
a  permutation  group  containing  in  addition  the  description of a root
system.  `cr*transpose(r)` should be  a Cartan matrix  for a finite Coxeter
group.  Each element of the coxeter group is represented as the permutation
it  induces on the roots,  coded as a permutation  of `1:2N` where we label
the  positive roots  by `1:N`,  and the  negative roots  by `N+1:2N` (these
permutations  do not  depend on  the actual  roots but  just on  the Cartan
matrix).  For Weyl groups  the ordering of  the roots is  very specific: we
order  the positive roots  by the sum  of their coefficients  on the simple
roots, and lexicographically according to the decomposition in simple roots
for  a given sum, and  order the negative roots  in the same order as their
negatives.
...
`rootdatum(cartan(t))`   can  be  simplified  to  `coxgroup(t)`,  giving  a
convenient way to get a Coxeter group of a given type.
...
Also,  instead  of  using  the  direct  sum  of  the  Cartan  matrices, the
`FiniteCoxeterGroup` associated to a direct sum of irreducible root systems
can be obtained as
...
Note that all the function for reflection groups are available for finite
Coxeter groups:
[`rank`](@ref),
[`semisimplerank`](@ref),
[`roots`](@ref),
...
```
"""
function PermRoot.degrees(W::ComplexReflectionGroup)
  get!(W,:degrees)do
...
```
"""
function PermRoot.codegrees(W::ComplexReflectionGroup)
...
[rich82](@cite)  there is a unique parabolic  subgroup `P` of `W` such that
`P` is finite and `w` is the longest element of `P`, and is central in `P`.
The  function  returns  `I`  such  that  `P==reflection_subgroup(W,I)`  and
...
A  theorem found independently by [deo89](@cite) and [dye90](@cite) is that
a  subgroup `H` of a Coxeter system  `(W,S)` generated by reflections has a
canonical  Coxeter  generating  set,  formed  of  the  `t  ∈  Ref(H)`  such
`length(W,tt')>length(W,t)`  for any `t'∈ Ref(H)`  different from `t`. This
...
using that by [lus76](@cite), the images of the roots of `W` in ``X(ZL_J)``
form  a root  system. The  function returns  `R` as  a reflection  group on
``X(ZL_J/ZG)``, with some extra attributes reflecting its origin
...
Coxeter  group, called the *affine Weyl group* `W̃` associated to `W`
...
(the  affine reflection with respect  to `α₀`) compared to  `W`. We can not
use  `0` as a label by default for  a generator of a Coxeter group (because
the default labels are used as indices, and indices start at 1 in Julia) so
we label it as `n+1` where `n` is the numbers of generators of `W`.
