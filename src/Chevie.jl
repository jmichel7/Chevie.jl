"""
This  is my effort to port to Julia the Chevie package from GAP3. I started
this  project at the end of 2018 and it  is still in flux so the package is
not  yet  registered.  If  you  see  anything  that needs improving in this
package,  please contact me or make an  issue or pull request in the GitHub
repository.
...
their  functionality is  automatically available  when you  use `Chevie`. In
other terms, `Chevie` is a meta-package for the following packages:
...
sits currently in `Chevie` but may become eventually separate packages:
...
to  implement  (such  as  character  tables  of  arbitrary  groups) `Chevie`
