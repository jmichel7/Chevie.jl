...
function Base.show(io::IO,s::Series)
  ...
  if s.spets == s.levi
    ...
    printTeX(io,"\$",s.d,"\$-series \$R^{",s.spets,"}_","{",s.levi,"}($n==",cname,")\$")
  end
  if haskey(s, :WGL)
...
Base.show(io::IO, ::MIME"text/latex", s::Series)=print(io,TeXs(s))
...
# if haskey(s, :eigen) f("\\hbox{eigen}", s.eigen) end
  ...
  f("\\hbox{family n⁰}", map(j->findfirst(k->j in k.charNumbers,
