...
# all timings are on dell-home; see chevie.g for GAP3 timings

#julia> @btime test_w0(7)
#1.3 108.103 ms (1789655 allocations: 166.39 MiB)
#good1.5 86.432 ms (519209 allocations: 125.44 MiB)
#1.6    100.107 ms (519465 allocations: 125.43 MiB)
#bad1.6 137.160 ms (1855942 allocations: 153.57 MiB)
...
#1.10β2 126.461 ms (1776191 allocations: 132.71 MiB)
#1.10rc3 131.801 ms (1776195 allocations: 132.71 MiB)
function test_w0(n)
  W=coxsym(n+1)
...
#  253.780 ms (2415862 allocations: 271.86 MiB) before lower
#1.1.1 426.050 ms (6728396 allocations: 500.72 MiB)
#good1.5 346.079 ms (4367402 allocations: 366.17 MiB)
#bad1.5  419.745 ms (8284628 allocations: 496.55 MiB)
#good1.6 358.302 ms (4374636 allocations: 366.22 MiB)
...
#1.10rc3 187.554 ms (1851568 allocations: 188.62 MiB)
#testmat(12)^2 in GAP3 best 0.37s in GAP4 0.3s
...
#1.5   678.376 ms (11307992 allocations: 681.21 MiB)
#good 1.5 470.366 ms (11317790 allocations: 681.36 MiB)
#1.6   710.814 ms (11305397 allocations: 681.19 MiB)
#good1.7 442.578 ms (11302895 allocations: 557.72 MiB)
#1.7.2 508.730 ms (23422968 allocations: 742.69 MiB)
#1.8   496.321 ms (11302897 allocations: 557.72 MiB)
#1.9.3 415.639 ms (11303721 allocations: 557.64 MiB)
#1.10β3 334.296 ms (11303721 allocations: 557.64 MiB)
#1.10rc3 400.549 ms (4046122 allocations: 336.15 MiB) # inline iterator
test_PermGroup(n::Int)=length(collect(symmetric_group(n)))
# GAP3 test_PermGroup(10);; takes 0.5s
#test_PermGroup:=n->Length(Elements(Group(List([1..n-1],i->(i,i+1)),())));

#julia> @btime test_elm()
#1.1 634.309 ms (5939476 allocations: 744.24 MiB)
#    516.881 ms (5945569 allocations: 1.08 GiB)
#1.3 790.355 ms (5947612 allocations: 1.08 GiB)
#1.5 842.683 ms (3018845 allocations: 1.04 GiB)
#good1.5 633.172 ms (3015031 allocations: 1.04 GiB)
...
#1.10rc3 745.679 ms (2947233 allocations: 948.39 MiB)
...
#bad1.6.3 2.090s (17149402 allocations: 2.77 GiB)
#1.7.2 1.314 s (13936367 allocations: 2.27 GiB)
#1.8   1.332 s (13934392 allocations: 2.27 GiB)
...
#1.10rc1 1.278 s (13494630 allocations: 2.32 GiB)
function test_kl(W)
  q=Pol([1],1)
...
#bad1.5 11.120s (365538047 allocations: 16.03 GiB)
#good1.5 7.269s (66263103 allocations: 11.57 GiB)
...
#1.10α 6.783 s (63825901 allocations: 9.27 GiB)
function test_kl1(W)
  q=Pol([1],1)
  H=hecke(W,q^2,rootpara=q)
  C=Cpbasis(H)
...
#1.10rc3 3.306 s (39520330 allocations: 5.07 GiB)
function test_kl2(W)
  el=elements(W)
  maximum(degree(KLPol(W,x,y)) for x in el, y in el)
end
# GAP3 takes 37s for F4
...
#1.10α 254.662 μs (6182 allocations: 516.09 KiB)
function test_b(W)
  B=BraidMonoid(W)
...
#1.10α 527.964 ms (3359892 allocations: 1006.48 MiB)
...
#1.10.β2 17.597 μs (414 allocations: 28.25 KiB)

# @btime inv(Frac.([x+y x-y;x+1 y+1])) setup=((x,y)=Mvp(:x,:y))
#bad1.6.3 187.233 μs (3941 allocations: 291.50 KiB)
...
#1.10rc1 152.979 μs (3486 allocations: 240.16 KiB)
